...
- (void)viewWillLayoutSubviews {
	...
	for (MWZoomingScrollView *page in _visiblePages) {
        ...
        CGRect previousBounds = CGRectMake(0, 0, 0, 0);
...
- (void)reloadData {
    ...
    for (int i = 0; i < (int)numberOfPhotos; i++) [_photos addObject:[NSNull null]];
    ...
    _currentPageIndex = MAX(0, MIN((int)_currentPageIndex, (int)numberOfPhotos - 1));
...
- (void)tilePages {
    ...
    if (iFirstIndex > (int)[self numberOfPhotos] - 1) iFirstIndex = [self numberOfPhotos] - 1;
    if (iLastIndex < 0) iLastIndex = 0;
    if (iLastIndex > (int)[self numberOfPhotos] - 1) iLastIndex = [self numberOfPhotos] - 1;
...
- (BOOL)isDisplayingPageForIndex:(NSUInteger)index {
	for (MWZoomingScrollView *page in _visiblePages)
		if (PAGE_INDEX(page) == (int)index) return YES;
	return NO;
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (index < 0) index = 0;
	if (index > (int)[self numberOfPhotos] - 1) index = [self numberOfPhotos] - 1;
