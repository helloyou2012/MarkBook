...
- (UIBezierPath *)layoutPath {
    ...
    const double endAngle = startAngle + TWO_M_PI * self.progress/self.max;
...
- (void)setProgress:(float)progress {
    //NSParameterAssert(progress >= 0 && progress <= self.max);
...
- (void)updateLabel {
    self.valueLabel.text = [self.numberFormatter stringFromNumber:@((int)self.progress)];
}
